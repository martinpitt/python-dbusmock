...
org.freedesktop.NetworkManager object, but no devices. You can
specify any property such as NetworkingEnabled,
WirelessEnabled etc. in "parameters".
'''
...
def load(mock, parameters):
    ...
    mock.AddProperties('',
                           {
                         'NetworkingEnabled':
                          parameters.get('NetworkingEnabled', True),
                         ...
                         'WimaxHardwareEnabled':
                          parameters.get('WimaxHardwareEnabled', True),
                         'WirelessEnabled':
                          parameters.get('WirelessEnabled', True),
                         'WirelessHardwareEnabled':
                          parameters.get('WirelessHardwareEnabled', True),
                         'WwanEnabled': parameters.get('WwanEnabled', False),
                         'WwanHardwareEnabled':
                          parameters.get('WwanHardwareEnabled', True)})
...
def AddEthernetDevice(self, device_name, iface_name, state):
    ...
    For valid state values, please visit
...
def AddWiFiDevice(self, device_name, iface_name, state):
    ...
    dev_obj.AddProperties('org.freedesktop.NetworkManager.Device',
                        ...
                        'State': dbus.UInt32(state,
                                                   variant_level=1),
                        'Interface': dbus.String(iface_name,
                                                   variant_level=1),
...
def AddAccessPoint(self, dev_path, ap_name, ssid, bssid,
    ...
    ssid, bssid, mode, frequency, rate, strength and security.
    For valid access point property values, please visit
    http://projects.gnome.org/NetworkManager/developers/api/09/
    spec.html#org.freedesktop.NetworkManager.AccessPoint
